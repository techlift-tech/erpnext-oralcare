        ...
        if filters.get("account") or (filters.get('party') and len(filters.party) == 1):
                ...
                if filters.get("account"):
                        ...
                        gle_currency = frappe.db.get_value(
                            "GL Entry",
                            {"party_type": filters.party_type, "party": filters.party[0],
                        ...
                        if gle_currency:
                                ...
                                account_currency = (
                                    None if filters.party_type in [
                                        "Employee",
                                        "Student",
                                        "Shareholder",
                                        "Member"] else frappe.db.get_value(
                                        filters.party_type,
                                        filters.party[0],
                                        "default_currency"))
...
def get_gl_entries(filters):
		...
		select
			...
			voucher_type,
                        case voucher_type when "Payment Entry" then (select payment_type from `tabPayment Entry`
                        where name = voucher_no) when "Journal Entry" then (select voucher_type from `tabJournal Entry`
                        where name = voucher_no) else "" END as entry_type,
...
def get_result_as_list(data, filters):
        ...
        for d in data:
                ...
                if balance > 0:
                        d['drcr'] = "Dr"
                if abs(balance - 0) <= 0.00001:
                        d['drcr'] = ""

                d['balance'] = abs(balance)
