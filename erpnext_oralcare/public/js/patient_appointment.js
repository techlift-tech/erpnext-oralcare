	...
	function show_availability() {
		...
		var d = new frappe.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "Link", options: "Medical Department", reqd:1, fieldname: "department", label: "Medical Department", read_only: 1},
				{ fieldtype: "Column Break"},
		...
		d.fields_dict["practitioner"].df.onchange = () => {
			if(d.get_value("practitioner") && d.get_value("practitioner") !== selected_practitioner){
				frappe.db.get_value("Healthcare Practitioner", {name: d.get_value("practitioner")},  "department")
				.then(function(value) {
					if(value && value.message && value.message.department) {
						d.set_values({
							"department": value.message.department
						});
					}
