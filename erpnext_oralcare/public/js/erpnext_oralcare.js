(function () {
	...
	(function() {
		...
		var store = fluxify.createStore({
			...
			actionCallbacks: {
				...
				update_cards: function(updater, cards) {
					var state = this;
					var _cards = cards
						.map(function (card) { return prepare_card(card, state); })
						.concat(this.cards)
				...
				add_card: function(updater, card_title, column_title) {
					...
					// for better ux
					var card = prepare_card(doc, state);
					card._disable_click = true;
					var cards = state.cards.concat( [card]);
					// remember the name which we will override later
					var old_name = doc.name;
					...
					if (field && !quick_entry) {
						return insert_doc(doc)
							.then(function(r) {
								...
								var index = state.cards.findIndex(function (card) { return card.name === old_name; });
								var card = prepare_card(updated_doc, state);
								var new_cards = state.cards.slice();
				...
				update_order: function(updater) {
					...
					var _cards = this.cards.slice();
					var _columns = this.columns.slice();
		...
		frappe.views.KanbanBoardColumn = function(column, wrapper) {
			...
			function make_cards() {
				...
				var filtered_cards_names = filtered_cards.map(function (card) { return card.name; });
		...
		function prepare_card(card, state, doc) {
			...
			for(var i = 0, list = state.board.fields_to_show; i < list.length; i += 1) {
				var field = list[i];
	...
	frappe.templates['kanban_card_new'] = '<div class="kanban-card-wrapper {{ disable_click }}" data-name="{{name}}">  <div class="kanban-card content">   <div class="kanban-card-title">    {{ title }}    <br>    {% for field in fields_string %}     {{ field }}     <br>    {% endfor %}   </div>   <div class="kanban-card-meta">   </div>  </div> </div> ';
