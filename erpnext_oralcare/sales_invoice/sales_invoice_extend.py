                ...
                if reference_dt in billable_healtcare_doctypes and reference_dt:
                        ...
                        if healthcare_doc:
                                if reference_dt == billable_healtcare_doctypes[0]:
                                        ...
                                        party = doctor
                                        sales_item.doctor_share, sales_item.admin_fees, sales_item.consumable_cost = get_doctor_commission(
                                                                                                                                           item_code,
                                                                                                                                           doctor,
                                                                                                                                           qty,
                                                                                                                                           amount)
...
def get_doctor_commission(item_code, doctor, qty, amount):
        ...
        consumable_cost = 0
        if doctor:
                commission_price = frappe.get_list(
                    'Item Commission List', filters={
                        'doctor_name': doctor, 'item_code': item_code}, fields=[
                ...
                if (not (commission_type and commission_value)):
                        return share_value, admin_fees, consumable_cost

                total_deduction = 0
                deductions = frappe.get_list(
                    'Deduction Before Commission', fields=[
                ...
                for deduction in deductions:
                        ...
                        if deduction_type == 'Percent':
                                if commission_type == 'Percent':
                                        ...
                                        deduction_to_add = ((qty * commission_value) * deduction_value) / 100
                ...
                admin_fees = total_deduction

                if commission_type == 'Percent':
                        item_deductions = frappe.get_list(
                            'Item Deduction Before Commission', filters={
                                'item': item_code}, fields=[
                                'type', 'value'])
                        ...
                        consumable_cost = total_deduction - admin_fees
                        ...
                        consumable_cost = 0
                        share_value = (qty*commission_value) - admin_fees
                        if share_value < 0:
                                frappe.throw(_("Final amount after deductions and commission is negative"))
...
def get_membership_card_from_customer(customer):
        membership_allotment = frappe.get_list(
            'Membership Allotment', filters={
                'primary_member': customer}, fields=[
        ...
        if membership_allotment and len(membership_allotment) == 1:
                ...
                secondary_member = frappe.get_all(
                    'Secondary Member', filters={
                        'member': customer}, fields=[
                        'name', 'parent'])
                if secondary_member and len(secondary_member) == 1:
                        membership_card_alotment = secondary_member[0].parent
                        membership_card_allotment_object = frappe.get_doc(
                            'Membership Allotment', membership_card_alotment)
        ...
        if patient:
                if patient.customer:
                        ...
                        patient_appointments = frappe.get_list(
                            "Patient Appointment", {'patient': patient.name, 'invoiced': False},
                                                ...
                                                if fee_validity_details:
                                                        for validity in fee_validity_details:
                                                                if validity['practitioner'] == patient_appointment_obj.practitioner:
                                                                        ...
                                                                        if not skip_invoice:
                                                                                validity['visits'] = 1
                                                                                validity['valid_till'] = patient_appointment_obj.appointment_date + \
                                        ...
                                        if not encounter_obj.appointment:
                                                ...
                                                if encounter_obj.practitioner:
                                                        service_item, practitioner_charge = service_item_and_practitioner_charge(
                                                                encounter_obj)
                                                        income_account = get_income_account(
                                                            encounter_obj.practitioner, encounter_obj.company)

                                                item_to_invoice.append(
                                                    {'reference_type': 'Patient Encounter',
                                                     'reference_name': encounter_obj.name, 'service': service_item,
                                                     'rate': practitioner_charge, 'income_account': income_account})
                        ...
                        if lab_tests:
                                for lab_test in lab_tests:
                                        ...
                                        if frappe.db.get_value("Lab Test Template",
                                                               lab_test_obj.template, "is_billable") == 1:
                                                item_to_invoice.append(
                                                    {'reference_type': 'Lab Test', 'reference_name': lab_test_obj.name,
                                                     'service': frappe.db.get_value(
                                                         "Lab Test Template", lab_test_obj.template, "item")})
                        ...
                        if procedures:
                                for procedure in procedures:
                                        ...
                                        if procedure_obj.procedure_template and(frappe.db.get_value(
                                                                                    "Clinical Procedure Template",
                                                                                    procedure_obj.procedure_template,
                                                                                    "is_billable") == 1):
                                                item_to_invoice.append({'reference_type': 'Clinical Procedure',
                                                                        'reference_name': procedure_obj.name,
                                                                        'service': frappe.db.get_value(
                                                                            "Clinical Procedure Template",
                        ...
                        procedures = frappe.get_list("Clinical Procedure",
                                                     {'patient': patient.name,
                                                      ...
                                                      'consumption_invoiced': False, 'consume_stock': True,
                                                      'status': 'Completed'})
                        if procedures:
                                service_item = get_healthcare_service_item('clinical_procedure_consumable_item')
                                if not service_item:
                                        msg = _(("Please Configure {0} in ").format(
                                ...
                                for procedure in procedures:
                                        ...
                                        item_to_invoice.append({'reference_type': 'Clinical Procedure',
                                                                ...
                                                                'rate': procedure_obj.consumable_total_amount,
			...
			io.left=1 and io.invoiced=0""", (patient.name))
                        if inpatient_services:
                                for inpatient_service in inpatient_services:
                                        ...
                                        service_unit_type = frappe.get_doc("Healthcare Service Unit Type", frappe.db.get_value(
